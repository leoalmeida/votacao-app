...
export class PautaService {
  private baseUrl:string = '/ms-pautas/v1/pautas';
  ...
  getAll(): void{
    this.http.get<Pauta[]>(`${this.baseUrl}`)
                  .subscribe(xs => this.pautasList.set(xs));
  }

  buascarPautasComVotos(idAssociado:number): void{
    this.http.get<Pauta[]>(`${this.baseUrl}/associado/${idAssociado}`)
                  .subscribe(xs => this.pautasList.set(xs));
  }

  //POST - "/"
  criarPauta(pauta: Pauta,iniciarSessao:boolean): boolean {
    console.log(`Solicitando nova votação: associado: ${pauta.idAssociado}, pauta: ${pauta.nome}.`);
