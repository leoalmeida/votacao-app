...
export class AssociadoService {
  private baseUrl:string = '/ms-associados/v1/associados';
  ...
  getById(idAssociado: number): Observable<Associado> {
    return this.http.get<Associado>(`${this.baseUrl}/${idAssociado}` )
  ...
  alterarAssociado(associado: Associado): Observable<Associado>  {
    ...
    return this.http.put<Associado>(`${this.baseUrl}/${associado.id}`, associado);
  }
  //DELETE - "/"
  removerAssociado(idAssociado: number): void {
    ...
    this.http.delete(`${this.baseUrl}/${idAssociado}`).subscribe({
        next: (res) => console.log(`Associado ${idAssociado} removido com sucesso.`),
